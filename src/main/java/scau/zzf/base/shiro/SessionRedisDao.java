...
public class SessionRedisDao extends EnterpriseCacheSessionDAO {
    ...
    @Autowired
    private RedisCacheUtil<Session> redisCacheUtil;
    // 创建session，保存到数据库
    @Override
    protected Serializable doCreate(Session session) {
        Serializable sessionId = super.doCreate(session);
//        RedisDb.setObject(sessionId.toString().getBytes(), sessionToByte(session));
        redisCacheUtil.setCacheObject(sessionId.toString(), session);
        return sessionId;
    }
...
//            }
            session = redisCacheUtil.getCacheObject(sessionId.toString());
        }
        return session;
    }
...
//        RedisDb.setObject(session.getId().toString().getBytes(), sessionToByte(session));
        redisCacheUtil.setCacheObject(session.getId().toString(), session);
    }
...
//        RedisDb.delString(session.getId() + "");
        redisCacheUtil.delete(session.getId().toString());
    }
